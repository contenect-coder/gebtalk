    ...
    try {
      ...
      try {
        ...
        if (jsMediaStream != null) {
          ...
          // Schedule aggressive retry to ensure playback starts even if
          // the initial play() was blocked by autoplay policy
          Future.delayed(const Duration(milliseconds: 300), () {
            try {
              if (elem.paused) {
                ...
                elem.muted = false;
                elem.play().toDart.catchError((_) => null);
                debugPrint('[WebRtcAudioSink] Retried play() after 300ms');
              }
            } catch (_) {}
          });
          Future.delayed(const Duration(milliseconds: 1000), () {
            try {
              if (elem.paused) {
                ...
                elem.muted = false;
                elem.play().toDart.catchError((_) => null);
                debugPrint('[WebRtcAudioSink] Retried play() after 1000ms');
              }
            } catch (_) {}
          });
